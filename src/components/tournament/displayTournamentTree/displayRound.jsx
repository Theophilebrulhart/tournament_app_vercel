...
export default function DisplayRound({
  round,
  maxRound,
  tournament,
  roundIndex,
  ...
  const generateTournamentRound = async (timePlan) => {
    ...
    if (
      ...
      tournament.tournamentRounds.length > roundIndex + 1
    ) {
      setDeleteRound(true);
      return;
    }
  ...
  const matches = round.matches.sort((a, b) => {
    if (
      a.teamsInMatch[0].rank + a.teamsInMatch[1].rank ===
      b.teamsInMatch[0].rank + b.teamsInMatch[1].rank
    )
      return a.field - b.field;
    return (
      a.teamsInMatch[0].rank +
      a.teamsInMatch[1].rank -
      (b.teamsInMatch[0].rank + b.teamsInMatch[1].rank)
    );
  });
  ...
  return (
    <>
      ...
      <div className="bg-red-500/10 flex flex-col gap-4">
        ...
        <button
          onClick={generateTournamentRound}
